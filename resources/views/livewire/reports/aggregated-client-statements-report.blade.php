<div>
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#1E293B]">كشوف العملاء المجمّعة</h1>
            <p class="text-sm text-gray-500 mt-1">ملخص كشف حساب لكل عميل/عملة ضمن الفترة — للتفاصيل الكاملة افتح كشف العميل.</p>
        </div>
    ...
    <div class="flex flex-wrap gap-4 mb-4">
        ...
        <div class="card px-4 py-2 text-sm">
            <span class="text-gray-500">{{ $cur }} — {{ $t['clients'] }} عميل:</span>
            <span class="font-mono font-bold text-[#1B6CA8] mr-2" dir="ltr">{{ number_format($t['balance'], 2) }}</span>
        </div>
        @endforeach
    </div>
    @endif
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="w-28 text-left" dir="ltr">فواتير</th>
                    <th class="w-28 text-left" dir="ltr">دفعات</th>
                    <th class="w-28 text-left" dir="ltr">تسويات</th>
                    <th class="w-32 text-left" dir="ltr">الرصيد</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="font-mono text-left text-sm" dir="ltr">{{ number_format($row['total_invoiced'], 2) }}</td>
                    ...
                    <td class="font-mono font-semibold text-left text-[#1B6CA8]" dir="ltr">{{ number_format($row['balance'], 2) }}</td>
                    <td class="text-center text-sm text-gray-500">{{ $row['movement_count'] }}</td>
                    <td>
                        <a href="{{ route('clients.statement', ['client' => $row['client_id'], 'date_from' => $dateFrom, 'date_to' => $dateTo]) }}" wire:navigate
