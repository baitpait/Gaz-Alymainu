<div>
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#1E293B]">تقرير المصروفات</h1>
            <p class="text-sm text-gray-500 mt-1">تفصيل كل مصروف ضمن الفترة المحددة.</p>
        </div>
    ...
    <div class="flex flex-wrap gap-4 mb-4">
        ...
        <div class="card px-4 py-2 text-sm">
            ...
            <span class="font-mono font-bold text-red-600 mr-2" dir="ltr">{{ number_format($total, 2) }}</span>
        </div>
        @endforeach
    </div>
    @endif
    ...
    @if(empty($rows))
    <div class="text-center py-16 text-gray-400">لا توجد مصروفات في هذه الفترة.</div>
    @else
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="font-mono font-semibold text-left text-red-600" dir="ltr">{{ number_format($row['amount'], 2) }}</td>
                    <td class="text-sm text-gray-500">{{ $row['recorded_by'] }}</td>
