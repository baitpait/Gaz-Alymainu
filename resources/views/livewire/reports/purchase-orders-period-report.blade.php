<div>
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#1E293B]">تقرير المشتريات</h1>
            <p class="text-sm text-gray-500 mt-1">أوامر شراء صادرة ضمن الفترة — مع حالة الدفع (FIFO).</p>
        </div>
    ...
    <div class="flex flex-wrap gap-4 mb-4">
        ...
        <div class="card px-4 py-2 text-sm">
            ...
            <span class="font-mono font-bold text-purple-600 mr-2" dir="ltr">{{ number_format($t['total'], 2) }}</span>
            <span class="text-gray-400">({{ $t['count'] }} أمر)</span>
        </div>
        @endforeach
    </div>
    @endif
    ...
    @if(empty($rows))
    <div class="text-center py-16 text-gray-400">لا توجد أوامر شراء صادرة في هذه الفترة.</div>
    @else
