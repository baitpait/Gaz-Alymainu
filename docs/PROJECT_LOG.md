...
## [2026-09-06 12:29] - تحصيل السائقين في الإدارة مع تعديل وحذف
- **الهدف:** عرض تحصيل السائقين في لوحة الإدارة مع إجراءات تعديل وحذف منطقي، دون المساس ببيانات الإنتاج.
- **التغييرات:**
  - رابط «التحصيل» في قائمة الغاز والمخزون.
  - `CashBoxService::updateCollection` / `voidCollection` مع حماية الصندوق.
  - Gates `update-collections` / `delete-collections` للمحاسب فأعلى؛ السائق يبقى إنشاء فقط.
  - اختبارات `CollectionPageTest`.
- **تنبيه:** لا migrate. لا مسح قاعدة. على السيرفر: `git pull --ff-only` + بناء الأصول + كاش العروض فقط.

---

## [2026-08-27 17:22] - توثيق جلسة 26–27 + نشر الإنتاج (مكتمل ✅)
- **الهدف:** إغلاق توثيق الجلسة بعد رفع `c0fe2e2` ومزامنة السيرفر.
- **التغييرات:**
  - وثيقة الإغلاق: `docs/27_SESSION_SALES_EXPENSES_MOBILE_SCROLL_2026_08_27_AR.md`
  - إغلاق افتتاح الجلسة في `docs/26_…`
  - إنتاج: `git pull` + `npm run build` + تصفية كاش العروض في `/home/sarfesak/public_html/gaz` فقط
- **تنبيه:** لا migrate. بقايا: أصناف شراء فقط، فجوة جرة 48/أيهم، فاتورة التقييم على الإنتاج.

---

## [2026-08-27 17:18] - إصلاح تمرير الموبايل تحت الشريط السفلي (كل الصفحات)
- **الهدف:** الزبون لا يستطيع النزول لأسفل الصفحة على الهاتف/APK بسبب شريط سفلي ثابت وقصّ flex.
- **التغييرات:** هيكل `app-shell` / `app-main-scroll`؛ حشو ديناميكي للشريط + بانر PWA + safe-area؛ اختبار `AppShellScrollLayoutTest`.
- **تنبيه:** يحتاج `npm run build` على السيرفر لأن `public/build` مستبعد من Git.
...
## [2026-08-27 16:59] - افتتاح جلسة عمل جديدة (أُغلقت ✅)
- **الهدف:** مراجعة المحادثة ووثائق 18–25 وبدء جلسة بسياق واضح وبقايا معلّقة.
- **التغييرات:** وثيقة افتتاح `docs/26_SESSION_KICKOFF_2026_08_27_AR.md`؛ الإغلاق في `docs/27_…`. Commit الإنتاج: `c0fe2e2`.
