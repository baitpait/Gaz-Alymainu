...
## [2026-08-13 13:05] - توقيت العرض فلسطين (القدس/الخليل) مع بقاء التخزين UTC
...
- **التغييرات:**
  ...
  - اختبارات `AppDateTimeTest`.
- **تنبيه:** على الإنتاج أضف `APP_DISPLAY_TIMEZONE=Asia/Jerusalem` ثم `config:clear` + `view:cache`. لا migrate.

---

## [2026-08-12 12:15] - صندوق الأدمن: دخول دفعات العملاء + خصم موردين/رواتب/مصروفات (كاش وشيك)
...
- **التغييرات:**
  ...
  - اختبارات `MainCashBoxTest` + تحديث `FinancialSummaryPageTest`.
- **تنبيه:** لا migrate. بعد النشر: `git pull` + `view:cache`. مصروفات الشركة بلا حقل method فتُخصم من النقد افتراضياً.

---

## [2026-08-12 11:57] - افتتاح جلسة عمل جديدة (قيد العمل 🔄)
- **الهدف:** مراجعة المحادثة ووثائق الجلسات 18–23 وبدء جلسة بسياق واضح وبقايا معلّقة.
- **التغييرات:** وثيقة افتتاح `docs/24_SESSION_KICKOFF_2026_08_12_AR.md`.
- **تنبيه:** آخر نشر مكتمل 2026-08-09 (`f97eb36` → `b28c1da` → `48626a4`). بقايا: أصناف شراء فقط، فجوة جرة 48/أيهم، التحقق من فاتورة التقييم على الإنتاج.
