...
- **`CashBoxService`** — صندوق كل سائق حسب العملة:
  ...
  - `mainBoxByCurrency()` — رصيد الصندوق الرئيسي بعد دخول/خروج (كاش/شيك).
  - `mainBoxLedger()` — دفتر حركات الصندوق الرئيسي.
...
```
مبيعات نقدية ─┐
تحصيلات نقدية ─┼─▶ صندوق السائق (نقدي) ──(سحب كاش)──▶┐
مصروفات السائق ◀┘                                    │
                                                     ├─▶ الصندوق الرئيسي (نقد/شيك)
دفعات عملاء (كاش/شيك) ───────────────────────────────┘
دفع موردين (كاش/شيك) ──┐
رواتب مدفوعة (كاش/شيك)─┼─▶ تُخصم من الصندوق الرئيسي
مصروفات شركة (نقد) ────┘

bank/transfer لا يدخلان رصيد الكاش/الشيك المادي.
تحصيلات/تسليمات شيكات السائق ──▶ تدخل شيكات الصندوق بعد السحب.
مبيعات على الحساب ──▶ تخصم من مخزون السائق فقط (لا تدخل الصندوق النقدي)
```

صفحة `/financial-summary` تعرض: الصندوق الرئيسي (بعد الصافي)، صناديق السائقين، **دفتر حركات الصندوق الرئيسي**، وآخر مصروفات السائقين.
...
| `update-driver-expenses` | المحاسب/المدير فقط (تعديل مبلغ/تصنيف/ملاحظات؛ زيادة المبلغ محدودة برصيد الكاش) |
| `delete-driver-expenses` | المحاسب/المدير فقط (حذف منطقي → يعود المبلغ لصندوق السائق) |
| `update-sales` | المحاسب/المدير فقط (تعديل كمية/سعر/ملاحظات من سجل المبيعات مع ضبط المخزون) |
| `update-collections` | المحاسب/المدير فقط (تعديل مبلغ/آلية/ملاحظات؛ التخفيض محدود برصيد صندوق السائق) |
| `delete-collections` | المحاسب/المدير فقط (حذف منطقي + حماية الصندوق إن كان الكاش/الشيك قد سُحب) |
...
- `/collections` (`collections.index`) — التحصيل المستقل (رابط في قائمة الإدارة؛ «كل السائقين» + تعديل/حذف منطقي للمحاسب؛ السائق إنشاء فقط).
- `/driver-expenses` (`driver-expenses.index`) — مصروفات السائق (فلاتر بحث/تصنيف/تاريخ + «كل السائقين» للمحاسب؛ تعديل للمحاسب).
- تمرير الموبايل: هيكل مشترك `app-shell` / `app-main-scroll` يحجز مساحة الشريط السفلي وبانر PWA (docs/27).
...
- **قواعد الدستور ذات الصلة:** أسماء الحقول التي يستخدمها التطبيق لا تُعاد تسميتها (توافق خلفي)؛ الحذف منطقي (`is_deleted`/`softDeletes`)؛ التخزين UTC والعرض بتوقيت فلسطين (`APP_DISPLAY_TIMEZONE=Asia/Jerusalem` عبر `App\Support\AppDateTime`) — انظر `docs/25_SESSION_MAIN_CASHBOX_AND_TIMEZONE_2026_08_13_AR.md`.
